...
class VirtualDisplayViewModel @Inject constructor(private val context: Application) : AndroidViewModel(context) {
    ...
    private fun injectInputEventToDisplay(event: InputEvent): Boolean {
        return try {
            inputManagerInstance?.let { im ->
                injectInputEventMethod?.invoke(im, event, 0) as? Boolean
            } ?: false
        } catch (e: Exception) {
    ...
    fun updateSurfaceView(surfaceView: SurfaceView) {
        ...
        surfaceView.setOnTouchListener { _, event: MotionEvent ->
            lastCreatedVirtualDisplay?.let { vDisplay ->
                event.setDisplayId(vDisplay.display.displayId)
                injectInputEventToDisplay(event)
            } ?: run {
        ...
        surfaceView.setOnKeyListener { _, keyCode, event ->
            lastCreatedVirtualDisplay?.let { vDisplay ->
                // todo may not be required; check once by removing this
    ...
    fun startAppInVirtualDisplay(surface: Surface) {
        ...
        launchAppInSpecificDisplay(getApplication(), settingsIntent, lastCreatedVirtualDisplay!!.display.displayId)
    }
    ...
    private fun setupInputChannel() {
        try {
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
                ...
                inputChannel =
                    createInputChannelMethod.invoke(displayManager, "VirtualDisplayInputChannel_${hashCode()}")
    ...
    companion object {
        ...
        fun launchAppInSpecificDisplay(
            ...
            val intentWithFlags = intent.apply {
                addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
//                addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK)
            }
            launchActivityInSpecificDisplay(context, displayId, intentWithFlags)
        }
